{{-- resources/views/admin/skins/create.blade.php --}}
<x-app-layout>
    <div class="max-w-3xl mx-auto px-4 lg:px-6 py-8 space-y-6">
        <div class="flex items-center justify-between gap-3">
            <div>
                ...
                <p class="text-sm text-slate-400">
                    Заповни поля й натисни «Створити».
                </p>
            </div>
        ...
        <div class="bg-slate-950/70 border border-slate-800 rounded-2xl p-5 space-y-4">
            <form action="{{ route('admin.skins.store') }}" method="POST" class="space-y-4">
                @csrf
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="text" name="weapon" value="{{ old('weapon') }}"
                               class="w-full bg-slate-950/80 border border-slate-700 rounded-xl px-3 py-2 text-sm text-slate-100 focus:outline-none focus:ring-2 focus:ring-yellow-400">
                        @error('weapon')
                            <p class="text-xs text-red-400 mt-1">{{ $message }}</p>
                        @enderror
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="text" name="rarity" value="{{ old('rarity') }}"
                               class="w-full bg-slate-950/80 border border-slate-700 rounded-xl px-3 py-2 text-sm text-slate-100 focus:outline-none focus:ring-2 focus:ring-yellow-400">
                        @error('rarity')
                            <p class="text-xs text-red-400 mt-1">{{ $message }}</p>
                        @enderror
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="text" name="float_value" value="{{ old('float_value') }}"
                               class="w-full bg-slate-950/80 border border-slate-700 rounded-xl px-3 py-2 text-sm text-slate-100 focus:outline-none focus:ring-2 focus:ring-yellow-400">
                    </div>

                    <div>
                        <label class="block text-xs font-semibold text-slate-400 mb-1">
                            Pattern (необов’язково)
                        </label>
                ...
                <div>
                    ...
                    <textarea name="description" rows="4"
                              class="w-full bg-slate-950/80 border border-slate-700 rounded-xl px-3 py-2 text-sm text-slate-100 focus:outline-none focus:ring-2 focus:ring-yellow-400">{{ old('description') }}</textarea>
                </div>
